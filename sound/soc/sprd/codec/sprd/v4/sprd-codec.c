			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SPRD_CODEC_MIC_BIAS:
		...
		if(on) {//peng.lee debug
			if(sprd_codec->micbias_v) {
				regulator_set_voltage(sprd_codec->micbias_v,2200000,2200000);
				sp_asoc_pr_dbg("set mainmic bias to 2.2v.\n");
			}
		}
		break;
					   ...
					   struct device *dev)
{
	...
	sprd_codec_power_get(dev, &sprd_codec->micbias_v, "VMICBIAS");
